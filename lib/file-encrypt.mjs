...
import { TimestampAddStream } from "./timestamp-add-stream.mjs";
...
export const encryptFile = async (inputFile, password, printProgressCallBack, updateProgressCallBack, { flagDelete, flagOverwrite, flagMove }) => {
  ...
  const inputFileModifiedDate = inputFileStats.mtime;
  const inputFileCreatedDate = inputFileStats.birthtime;
  ...
  const createdDateStream = new TimestampAddStream(inputFileCreatedDate);
  const modifiedDateStream = new TimestampAddStream(inputFileModifiedDate);
  ...
  await pipeline(inputStream, createdDateStream, modifiedDateStream, compressStream, encryptStream, outputStream);
